...
class CarpetMap:
    ...
    def get_color_at_coords(self, coords:np.array) -> np.array:
        ...
        y_indices = np.ceil(self.grid.shape[0] - 1  - (coords[:,1] / self.cell_size)).astype(np.int)

        # avoid indexing points which are out of bounds
        in_bounds = (
            (x_indices >= 0) &
            (x_indices < self.grid.shape[1]) &
            (y_indices >= 0) &
            (y_indices < self.grid.shape[0])
        )
        colors[in_bounds] = self.grid[y_indices[in_bounds], x_indices[in_bounds]]
